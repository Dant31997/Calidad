...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 10, $this->convertirUTF8('INFORME DE PETICIONES DE ESPACIOS'), 0, 1, 'C');
    ...
    function TableHeader() {
        ...
        $this->Cell(50, 7, $this->convertirUTF8('Espacio solicitado'), 1, 0, 'C', true);
        $this->Cell(25, 7, $this->convertirUTF8('Fecha'), 1, 0, 'C', true);
        $this->Cell(35, 7, $this->convertirUTF8('Solicitante'), 1, 0, 'C', true);
        $this->Cell(25, 7, 'Estado', 1, 0, 'C', true);
        $this->Cell(20, 7, $this->convertirUTF8('Hora inicio'), 1, 0, 'C', true);
        $this->Cell(20, 7, $this->convertirUTF8('Hora fin'), 1, 0, 'C', true);
        $this->Ln();
    }
    ...
    function addConclusions() {
        ...
        $text = "• Un " . number_format($this->porcentajeSinRevisar, 2) . "% de las peticiones de espacios se encuentran sin revisar. ";

        if ($this->porcentajeSinRevisar > 30) {
            $text .= "Se recomienda agilizar el proceso de revisión para mejorar los tiempos de respuesta y la gestión de los espacios.";
        } else {
        ...
        if ($tasaAprobacion < 50) {
            $text .= "Este porcentaje es bajo y puede indicar una alta demanda o criterios muy estrictos para la asignación de espacios.";
        } elseif ($tasaAprobacion > 90) {
            $text .= "Este porcentaje es muy alto y podría ser necesario revisar los criterios de aprobación para garantizar un uso eficiente de los espacios.";
        } else {
            $text .= "Este porcentaje se encuentra en un rango adecuado, reflejando un balance entre disponibilidad y control de los espacios.";
        }
        $this->MultiCell(0, 6, $this->convertirUTF8($text), 0, 'J');
        $this->Ln(3);
        ...
        $recomendaciones = [
            ...
            "Realizar un análisis mensual de los espacios más solicitados para anticipar necesidades y optimizar su uso.",
            "Revisar los criterios de aprobación/rechazo para mantener un balance adecuado en la asignación de espacios.",
            "Considerar la implementación de un calendario de disponibilidad de espacios visible para todos los usuarios."
        ];
...
$sqlTotales = "SELECT estado_peticion, COUNT(*) as total FROM peticiones_espacios GROUP BY estado_peticion";
...
$sql = "SELECT id, nom_espacio, pide, estado_peticion, fecha_entrega, hora_entrega, hora_regreso 
        FROM peticiones_espacios 
...
if ($resultado->num_rows > 0) {
    ...
    while ($fila = $resultado->fetch_assoc()) {
        ...
        $pdf->Cell(50, 7, $pdf->convertirUTF8($fila['nom_espacio']), 1, 0, 'L', $fill);
        $pdf->Cell(25, 7, $fila['pide'], 1, 0, 'C', $fill);
        $pdf->Cell(35, 7, $pdf->convertirUTF8($fila['estado_peticion']), 1, 0, 'L', $fill);
        $pdf->Cell(25, 7, $pdf->convertirUTF8($fila['fecha_entrega']), 1, 0, 'C', $fill);
...
if ($resultado->num_rows > 0) {
    ...
    while ($fila = $resultado->fetch_assoc()) {
        ...
        $pdf->Cell(25, 7, $fila['pide'], 1, 0, 'C', $fill);
        $pdf->Cell(35, 7, $pdf->convertirUTF8($fila['estado_peticion']), 1, 0, 'L', $fill);
        $pdf->Cell(25, 7, $fila['fecha_entrega'], 1, 0, 'C', $fill);
        $pdf->Cell(20, 7, $fila['hora_entrega'], 1, 0, 'C', $fill);
        $pdf->Cell(20, 7, $fila['hora_regreso'], 1, 0, 'C', $fill);
        $pdf->Ln();
        $fill = !$fill;
...
$pdf->Output('Informe_Peticiones_Espacios.pdf', 'I');
